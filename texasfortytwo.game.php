...
class TexasFortyTwo extends Table {
  ...
  private function initializePlayers($players) {
    ...
    $default_colors = array_slice(self::POSSIBLE_PLAYER_COLORS, 0, count($players));
    $rows = array();
  ...
  */
  ...
  function stNextPlayer() {
    ...
    if ($this->dominoes->countCardInLocation('table') == 4) {
      ...
      $this->dominoes->moveAllCardsInLocation(
        'table', 'cardswon', null, $best_value_player_id);

      // Notify
      // Note: we use 2 notifications here in order we can pause the display
      // during the first notification before we move all cards to the winner
      // (during the second)
      $players = self::loadPlayersBasicInfos();
  ...
  function stEndHand() {
    ...
    foreach ( $player_to_points as $player_id => $points ) {
      if ($points != 0) {
        ...
        self::notifyAllPlayers("points",
          clienttranslate('${player_name} did not get any hearts'), array(
            'player_id' => $player_id,
            'player_name' => $players [$player_id] ['player_name']
          ));
      }
  ...
  /*
    ...
    This method is called each time it is the turn of a player who has quit the
    game (= "zombie" player).
    You can do whatever you want in order to make sure the turn of this player
    ends appropriately (ex: pass).
  */
  ...
  /*
    ...
    Once your game is on BGA, this method is called everytime the system detects
    a game running with your old database scheme.
    In this case, if you change your Database scheme, you just have to apply the
    needed changes in order to update the game database and allow the game to
    continue to run with your new version.
  */
  ...
  function upgradeTableDb( $from_version )
  {
    ...
    // TODO(sdspikes): how do you get the version? once released we probably
    // need this in case we change the schema later
